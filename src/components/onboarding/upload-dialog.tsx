...
export function UploadDialog({ open, onOpenChange, workspaceId, onUploadSuccess }: UploadDialogProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      console.log(`📤 [Upload Dialog] Uploading ${(file.size / 1024 / 1024).toFixed(2)}MB to Supabase`);

      // Direct upload to Supabase (no size limits)
      // This function will:
      // 1. Request signed URL (creates placeholder case if needed)
      // 2. Upload directly to Supabase S3
      // 3. Return file path for webhook processing
      const uploadResult = await uploadFile(
        file,
        workspaceId,
        '', // caseId will be auto-generated by signed-url endpoint
        'case-documents',
        (progress) => {
