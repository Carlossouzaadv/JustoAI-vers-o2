...
// e pode usar bibliotecas com workers/dependências nativas
...
// ================================================================
// EXTRATOR DE PDF COM CASCATA (sem canvas/workers)
// ================================================================
async function extractTextFromPDF(pdfPath: string): Promise<string> {
  log(`${ICONS.RAILWAY} ${ICONS.PDF}`, 'Iniciando extração de texto do PDF', { path: pdfPath });

  try {
    // Tentar com pdf-parse (puro Node.js, sem canvas)
    ...
    try {
      ...
      // Tentar com pdfjs-dist (CommonJS require para evitar webpack bundling)
      log(`${ICONS.RAILWAY} ${ICONS.PDF}`, 'Tentando método 2: pdfjs-dist');

      try {
        const pdfjs = require('pdfjs-dist/legacy/build/pdf.js');
        pdfjs.GlobalWorkerOptions = pdfjs.GlobalWorkerOptions || {};
        pdfjs.GlobalWorkerOptions.workerSrc = null; // Desabilita workers
