// ================================================================
// PROCESS UPLOAD API - PROXY to Railway Backend
// ================================================================
...
export async function POST(request: NextRequest) {
  try {
    console.log('📤 [Vercel Proxy] Proxying process upload to Railway:', `${RAILWAY_FULL_URL}/api/process/upload`);

    // Clone the request to avoid consuming it twice
    const clonedRequest = request.clone();

    // Forward the entire request (including FormData/multipart) to Railway
    const response = await fetch(`${RAILWAY_FULL_URL}/api/process/upload`, {
      method: 'POST',
      headers: {
